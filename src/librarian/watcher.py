...
class IndexingPipeline:
    ...
    def __init__(self, max_workers: int = 12):
        ...
        # Initialize DB with correct dimension from embedding engine
        self.db = LanceDBManager()
        embedding_dim = self.embeddings.dimension
        self.db.set_dimension(embedding_dim)
        logger.info(f"Initialized pipeline with embedding dimension: {embedding_dim}")
